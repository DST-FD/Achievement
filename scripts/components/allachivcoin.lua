...
local function currentfestive(self,festive) local c = 0 if festive then c=1 end self.inst.currentfestive:set(c) end
...
local function currentdisguise(self,disguise) local c = 0 if disguise then c=1 end self.inst.currentdisguise:set(c) end
...
function allachivcoin:OnLoad(data)
    ...
    self.festive = data.festive or false
    self.minemaster = data.minemaster or false
    self.disguise = data.disguise or false
    self.nanobots = data.nanobots or false
    self.fastworker = data.fastworker or false
end

--Festive Loot Drop
local function festiveloot(victim)
    if victim.components.lootdropper and (victim.components.freezable or victim:HasTag("monster")) then
        local chance = math.random()
        --WinterFood
        if chance <= 0.20 then
            local loottype = math.random(9)
            local winterloot = SpawnPrefab("winter_food"..tostring(loottype))
            victim.components.lootdropper:FlingItem(winterloot)
        --Ornamental
        elseif chance <= 0.21 then
            --Plains
            if math.random() <= 0.5 then
                local loottype = math.random(12)
                local winterloot = SpawnPrefab("winter_ornament_plain"..tostring(loottype))
                victim.components.lootdropper:FlingItem(winterloot)
            --Fancy
            else
                local loottype = math.random(8)
                local winterloot = SpawnPrefab("winter_ornament_fancy"..tostring(loottype))
                victim.components.lootdropper:FlingItem(winterloot)
            end
        end
        --Bosses
        if victim.prefab == "bearger" then
            local winterloot = SpawnPrefab("winter_ornament_boss_bearger")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "deerclops" then
            local winterloot = SpawnPrefab("winter_ornament_boss_deerclops")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "moose" then
            local winterloot = SpawnPrefab("winter_ornament_boss_moose")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "dragonfly" then
            local winterloot = SpawnPrefab("winter_ornament_boss_dragonfly")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "beequeen" then
            local winterloot = SpawnPrefab("winter_ornament_boss_beequeen")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "toadstool" then
            local winterloot = SpawnPrefab("winter_ornament_boss_toadstool")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "antlion" then
            local winterloot = SpawnPrefab("winter_ornament_boss_antlion")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "fuelweaver" then
            local winterloot = SpawnPrefab("winter_ornament_boss_fuelweaver")
            victim.components.lootdropper:FlingItem(winterloot)
        elseif victim.prefab == "leif" and math.random() < 0.3 then
            local loottype = math.random(6)
            local winterloot = SpawnPrefab("halloween_ornament_"..tostring(loottype))
            victim.components.lootdropper:FlingItem(winterloot)
        end
    end
end
...
function allachivcoin:hungerrateupcoin(inst)
    if self.coinamount >= allachiv_coinuse["hungerrateup"] and self.hungerrateupamount < 48 then
...
function allachivcoin:speedupcoin(inst)
    if self.coinamount >= allachiv_coinuse["speedup"] and self.speedupamount < _G.SPEEDLIM then
...
function allachivcoin:absorbupcoin(inst)
    if self.coinamount >= allachiv_coinuse["absorbup"] and self.absorbupamount < 19 then
...
function allachivcoin:critcoin(inst)
    if self.coinamount >= allachiv_coinuse["crit"] and self.crit < 20 then
...
function allachivcoin:doubledropfn(inst)
    inst:ListenForEvent("killed", function(inst, data)
        if self.doubledrop and data.victim.components.lootdropper then
            ...
            if self.festive and not IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST) then
                festiveloot(data.victim)
            end
        end
    end)
end
...
function allachivcoin:minemasterfn(inst)
    inst:ListenForEvent("working", function(inst, data)
        if self.minemaster and data.target and (data.target:HasTag("boulder") or 
        ...
        if self.nanobots then
            ...
            if inventory then
                for k, v in pairs(inventory.equipslots) do
                    if v.components.finiteuses  and (v.prefab ~= "orangestaff" and
                                                     v.prefab ~= "icestaff" and
                                                     v.prefab ~= "firestaff" and
                                                     not v.components.spellcaster) then
                        local p = v.components.finiteuses.current
                        p = math.min(p + (math.min(240,v.components.finiteuses.total) / 240), v.components.finiteuses.total)
                        v.components.finiteuses:SetPercent(p / v.components.finiteuses.total)

                    elseif v.components.armor then
                        local p = v.components.armor.condition
                        p = math.min(p + 3, v.components.armor.maxcondition)
                        v.components.armor:SetPercent(p / v.components.armor.maxcondition)
                    end
                end
            end
        end
...
--书籍阅读获取
function allachivcoin:disguisecoin(inst)
    if self.disguise ~= true and self.coinamount >= allachiv_coinuse["disguise"] then
        self.disguise = true
        self:coinDoDelta(-allachiv_coinuse["disguise"])
        self:disguisefn(inst)
        self:ongetcoin(inst)
    end
end

--书籍阅读效果
function allachivcoin:disguisefn(inst)
    if self.disguise then
        inst:AddTag("perkdisguise")
    end
end
...
--书籍阅读获取
function allachivcoin:festivecoin(inst)
    if self.festive ~= true and self.coinamount >= allachiv_coinuse["festive"] then
        self.festive = true
        self:coinDoDelta(-allachiv_coinuse["festive"])
        self:festivefn(inst)
...
--书籍阅读效果
function allachivcoin:festivefn(inst)
    if self.festive then
        inst:AddTag("festive")
    end
end

function allachivcoin:festivefndrop(inst)
    inst:ListenForEvent("killed", function(inst, data)
        if data.victim.components.lootdropper and self.festive and not IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST) then
            festiveloot(data.victim)
        end
    end)
end
...
function allachivcoin:removecoin(inst)
    ...
    if self.masterchef then returncoin = returncoin + allachiv_coinuse["masterchef"] end
    if self.festive then returncoin = returncoin + allachiv_coinuse["festive"] end
    if self.minemaster then returncoin = returncoin + allachiv_coinuse["minemaster"] end
    if self.disguise then returncoin = returncoin + allachiv_coinuse["disguise"] end
    if self.nanobots then returncoin = returncoin + allachiv_coinuse["nanobots"] end
...
function allachivcoin:resetbuff(inst)
    ...
    inst:RemoveTag("perkdisguise")
    inst:RemoveTag("fastworker")
